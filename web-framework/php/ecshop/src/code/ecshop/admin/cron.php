...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        $cron_code = filter_compile($_POST['cron_code']);
        $cron_name = filter_compile($_POST['cron_name']);
        $cron_desc = filter_compile($_POST['cron_desc']);
        $cron_run_once = filter_compile($_POST['cron_run_once']);
        $cron_desc = filter_compile($_POST['cron_desc']);
        $sql = "INSERT INTO " . $ecs->table('crons') . " (cron_code, cron_name, cron_desc, cron_config, nextime, day, week, hour, minute, run_once, allow_ip, alow_files)" .
               "VALUES ('$cron_code', '$cron_name', '$cron_desc', '$cron_config', '$next', '$cron_day', '$cron_week', '$cron_hour', '$cron_minute', '$cron_run_once', '$_POST[allow_ip]', '$_POST[alow_files]')";
        $db->query($sql);
