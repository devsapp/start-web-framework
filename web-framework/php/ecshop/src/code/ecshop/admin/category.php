...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $cat['keywords']     = !empty($_POST['keywords'])     ? filter_compile($_POST['keywords'])     : '';
    $cat['cat_desc']     = !empty($_POST['cat_desc'])     ? filter_compile($_POST['cat_desc'])     : '';
    $cat['measure_unit'] = !empty($_POST['measure_unit']) ? filter_compile($_POST['measure_unit']) : '';
    $cat['cat_name']     = !empty($_POST['cat_name'])     ? filter_compile($_POST['cat_name'])     : '';
    $cat['show_in_nav']  = !empty($_POST['show_in_nav'])  ? intval($_POST['show_in_nav']): 0;
    $cat['style']        = !empty($_POST['style'])        ? filter_compile($_POST['style'])        : '';
    $cat['is_show']      = !empty($_POST['is_show'])      ? intval($_POST['is_show'])    : 0;
    $cat['grade']        = !empty($_POST['grade'])        ? intval($_POST['grade'])      : 0;
    $cat['filter_attr']  = !empty($_POST['filter_attr'])  ? implode(',', array_unique(array_diff($_POST['filter_attr'],array(0)))) : 0;
    $cat['is_top']       = !empty($_POST['is_top'])       ? intval($_POST['is_top'])    : 0;
    $cat['is_goods']     = !empty($_POST['is_goods'])     ? intval($_POST['is_goods'])   : 0;
    $cat['cat_recommend']  = !empty($_POST['cat_recommend'])  ? $_POST['cat_recommend'] : array();
    $cat['cate_img_url']  = !empty($_POST['cate_img_url'])  ? $_POST['cate_img_url'] : '';
    ...
    **/
    if($_FILES['cate_img']){
        $cat['cate_img'] = $image->upload_image($_FILES['cate_img']);
    }
    if($_FILES['cate_img']['name'] == ''){
        $cat['cate_img'] = $cat['cate_img_url']; // 外链图片
    }
...
if ($_REQUEST['act'] == 'update')
{
    ...
    **/
    ...
    if($_FILES['cate_img']['name'] == '' && !empty($cat['cate_img_url'])){ // 使用外链上传
        $cat['cate_img'] = $cat['cate_img_url'];
    }
