...
$region_arr = area_list(1);
foreach ($region_arr as $k1 => $v1) {
    $region_arr_v1 =area_list($v1['region_id']);
    $regionJson[$k1]['id'] = $v1['region_id'];
    $regionJson[$k1]['value'] = $v1['region_name'];
    foreach ($region_arr_v1 as $k2 => $v2) {
        $region_arr_v2 =area_list($v2['region_id']);
        $regionJson[$k1]['children'][$k2]['id'] = $v2['region_id'];
        $regionJson[$k1]['children'][$k2]['value'] = $v2['region_name'];
        $regionJson[$k1]['children'][$k2]['parentId'] = $v1['region_id'];
        foreach ($region_arr_v2 as $k3 => $v3) {
            $regionJson[$k1]['children'][$k2]['children'][$k3]['id'] = $v3['region_id'];
            $regionJson[$k1]['children'][$k2]['children'][$k3]['value'] = $v3['region_name'];
            $regionJson[$k1]['children'][$k2]['children'][$k3]['parentId'] = $v2['region_id'];
        }
    }
}
$json_string = json_encode($regionJson);
// 写入文件
file_put_contents(ROOT_PATH.'/js/region.json', $json_string);
