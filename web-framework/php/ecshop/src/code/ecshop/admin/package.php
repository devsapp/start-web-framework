...
elseif ($_REQUEST['act'] =='insert')
{
    ...
    if(!empty($_FILES['package_image']['type'])){
        $path = dirname(__DIR__).'/images/';
        $file_name = md5(time()).'.jpg';
        $res = move_uploaded_file($_FILES['package_image']['tmp_name'],$path.$file_name);
        ...
        $images = $https.$_SERVER['HTTP_HOST']."/"."images/".$file_name;
    }
    if(!empty($_POST['package_image_url'])){
        $images = $_POST['package_image_url'];
    }
    /* 插入数据 */
    $record = array('act_name'=>filter_compile($_POST['package_name']), 'act_desc'=>filter_compile($_POST['desc']),
                    'act_type'=>GAT_PACKAGE, 'start_time'=>$_POST['start_time'],
                    'end_time'=>$_POST['end_time'], 'package_image'=>$images,'is_finished'=>0, 'ext_info'=>serialize($info));
    ...
    admin_log(filter_compile($_POST['package_name']),'add','package');
...
elseif ($_REQUEST['act'] =='update')
{
    ...
    $sql = "SELECT COUNT(*) ".
           " FROM " . $ecs->table('goods_activity').
           " WHERE act_type='" . GAT_PACKAGE . "' AND act_name='" . filter_compile($_POST['package_name']) . "' AND act_id <> '" .  $_POST['id'] . "'" ;
    if ($db->getOne($sql))
    {
        sys_msg(sprintf($_LANG['package_exist'],  filter_compile($_POST['package_name'])) , 1);
    }
    ...
    if(empty($images)){
        /* 更新数据 */
        $record = array('act_name' => filter_compile($_POST['package_name']), 'start_time' => $_POST['start_time'], 'end_time' => $_POST['end_time'],
            'act_desc' => filter_compile($_POST['desc']), 'ext_info'=>serialize($info));
    }else{
        /* 更新数据 */
        $record = array('act_name' => filter_compile($_POST['package_name']), 'package_image'=>$images,'start_time' => $_POST['start_time'], 'end_time' => $_POST['end_time'],
            'act_desc' => filter_compile($_POST['desc']), 'ext_info'=>serialize($info));
    ...
    admin_log(filter_compile($_POST['package_name']),'edit','package');
