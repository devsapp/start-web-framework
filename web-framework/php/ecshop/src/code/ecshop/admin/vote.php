...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $vote_name = filter_compile($_POST['vote_name']);
    /* 查看广告名称是否有重复 */
    ...
    if ($db->getOne($sql) == 0)
    {
        ...
        VALUES ('$vote_name', '$start_time', '$end_time', '$_POST[can_multi]', '0')";
        $db->query($sql);

        $new_id = $db->Insert_ID();

        /* 记录管理员操作 */
        admin_log($vote_name, 'add', 'vote');
        ...
        sys_msg($_LANG['add'] . "&nbsp;" .$vote_name . "&nbsp;" . $_LANG['attradd_succed'],0, $link);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    sys_msg($_LANG['edit'] .' '.$vote_name.' '. $_LANG['attradd_succed'], 0, $link);
}
/*------------------------------------------------------ */
//-- 调查选项列表页面
/*------------------------------------------------------ */
