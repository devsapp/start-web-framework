...
{
    $type = empty($_POST['type']) ? '' : filter_compile($_POST['type']);
    ...
    // 更新移动端env配置
    $arr = array(
        'MAIL_HOST' => isset($_POST['value']['501'])      ?   filter_compile($_POST['value']['501']) : '',
        'MAIL_PORT' => isset($_POST['value']['502'])      ?   filter_compile($_POST['value']['502']) : '',
        'MAIL_USERNAME' => isset($_POST['value']['503'])      ?   filter_compile($_POST['value']['503']) : '',
        'MAIL_PASSWORD' => isset($_POST['value']['504'])      ?   filter_compile($_POST['value']['504']) : '',
        'MAIL_FROM_ADDRESS' => isset($_POST['value']['505'])      ?   filter_compile($_POST['value']['505']) : '',
        );
...
//    var_dump($_POST['smtp_pass']);die;
    if (!$_POST['smtp_pass']){
        $sql = "SELECT value FROM " . $ecs->table('shop_config') .
            " WHERE `code` = 'smtp_pass'";
        $_CFG['smtp_pass'] = $db->getOne($sql);
    }else{
