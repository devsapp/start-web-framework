...
if ($_REQUEST['act'] == 'insert')
{
    /* 权限判断 */
    admin_priv('pack');
    $pack_handle = filter_compile($_POST['pack_name']);
    /*检查包装名是否重复*/
    $is_only = $exc->is_only('pack_name', $pack_handle);

    if (!$is_only)
    {
        sys_msg(sprintf($_LANG['packname_exist'], stripslashes($pack_handle)), 1);
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('pack')."(pack_name, pack_fee, free_money, pack_desc, pack_img)
            VALUES ('$pack_handle', '$_POST[pack_fee]', '$_POST[free_money]', '$_POST[pack_desc]', '$img_name')";
    $db->query($sql);
    ...
    sys_msg($pack_handle.$_LANG['packadd_succed'],0, $link);
    admin_log($pack_handle,'add','pack');
...
if ($_REQUEST['act'] == 'update')
{
    ...
    admin_priv('pack');
    $pack_handle = filter_compile($_POST['pack_name']);
    if ($pack_handle != $_POST['old_packname'])
    {
        /*检查品牌名是否相同*/
        $is_only = $exc->is_only('pack_name', $pack_handle, $_POST['id']);
    ...
    $param = "pack_name = '$pack_handle', pack_fee = '$_POST[pack_fee]', free_money= '$_POST[free_money]', pack_desc = '$_POST[pack_desc]' ";
    /* 处理图片 */
    ...
    if ($exc->edit($param ,  $_POST['id']))
    {
        ...
        $note = sprintf($_LANG['packedit_succed'], $pack_handle);
        sys_msg($note, 0, $link);
        admin_log($pack_handle, 'edit', 'pack');
    }
    else
    {
        die($db->error());
