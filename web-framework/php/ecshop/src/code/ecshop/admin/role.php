...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('role')." (role_name, action_list, role_describe) ".
           "VALUES ('".filter_compile($_POST['user_name'])."','$act_list','".filter_compile($_POST['role_describe'])."')";
    ...
    /* 记录管理员操作 */
    admin_log(filter_compile($_POST['user_name']), 'add', 'role');
 }
...
elseif ($_REQUEST['act'] == 'update')
{
    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " .$ecs->table('role'). " SET action_list = '$act_list', role_name = '".filter_compile($_POST['user_name'])."', role_describe = '".filter_compile($_POST['role_describe'])." ' ".
    ...
    sys_msg($_LANG['edit'] . "&nbsp;" . filter_compile($_POST['user_name']) . "&nbsp;" . $_LANG['action_succeed'], 0, $link);
}
