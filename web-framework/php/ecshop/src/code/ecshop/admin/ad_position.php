...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 对POST上来的值进行处理并去除空格 */
    $position_name = !empty($_POST['position_name']) ? filter_compile($_POST['position_name']) : '';
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $preg = "<,>/?~`!@#%^&*()+|\='";
    if(similar_text($position_desc,$preg)>0)
    {
        sys_msg($_LANG['posit_desc_length'], 1, array(), false);
    }
    /* 查看广告位是否与其它有重复 */
