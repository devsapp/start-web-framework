...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $link_name  = (!empty($_POST['link_name']))  ? sub_str(filter_compile($_POST['link_name']), 250, false) : '';
    ...
    if ($exc->num("link_name", $link_name) == 0)
    {
        ...
        if (strpos(compile_str($_POST['link_url']), 'http://') === false && strpos(compile_str($_POST['link_url']), 'https://') === false)
        {
            $link_url = 'http://' . filter_compile($_POST['link_url']);
        }
        else
        {
            $link_url = filter_compile($_POST['link_url']);
        }
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $show_order = (!empty($_POST['show_order'])) ? intval($_POST['show_order']) : 0;
    $link_name  = (!empty($_POST['link_name']))  ? filter_compile($_POST['link_name'])    : '';
    ...
    {
        $url_logo = filter_compile($_POST['url_logo']);
        $link_logo = ", link_logo = '$url_logo'";
    }
    else
    {
        /* 如果是文字链接, LOGO为链接的名称 */
