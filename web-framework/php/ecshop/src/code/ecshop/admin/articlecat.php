...
elseif ($_REQUEST['act'] == 'insert')
{
    /* 权限判断 */
    admin_priv('article_cat');
    $cat_name = filter_compile($_POST['cat_name']);
    $keywords = filter_compile($_POST['keywords']);
    $cat_desc = filter_compile($_POST['cat_desc']);
    $sort_order = intval($_POST['sort_order']);
    /*检查分类名是否重复*/
    $is_only = $exc->is_only('cat_name', $cat_name);

    if (!$is_only)
    {
        sys_msg(sprintf($_LANG['catname_exist'], stripslashes($cat_name)), 1);
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('article_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav)
           VALUES ('$cat_name', '$cat_type',  '$cat_desc','$keywords', '$_POST[parent_id]', '$sort_order', '$_POST[show_in_nav]')";
    $db->query($sql);
    ...
    if($_POST['show_in_nav'] == 1)
    {
        ...
        $vieworder += 2;
        //显示在自定义导航栏中
        $sql = "INSERT INTO " . $ecs->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $cat_name . "', 'a', '" . $db->insert_id() . "','1','$vieworder','0', '" . build_uri('article_cat', array('acid'=> $db->insert_id()), $cat_name) . "','middle')";
        $db->query($sql);
    }

    admin_log($cat_name,'add','articlecat');
    ...
    clear_cache_files();
    sys_msg($cat_name.$_LANG['catadd_succed'],0, $link);
}
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $sort_order = intval($_POST['sort_order']);
    /*检查重名*/
    if ($cat_name != filter_compile($_POST['old_catname']))
    {
        $is_only = $exc->is_only('cat_name', $cat_name, $_POST['id']);
    ...
    {
        sys_msg(sprintf($_LANG['parent_id_err'], stripslashes($cat_name)), 1);
    }
    ...
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$cat_desc', keywords='$keywords',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$sort_order', show_in_nav = '$_POST[show_in_nav]'",  $_POST['id']))
    {
        if($cat_name != $dat['cat_name'])
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $ecs->table('nav') . " SET name = '" . $cat_name . "' WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'";
            $db->query($sql);
        }
        if($_POST['show_in_nav'] != $dat['show_in_nav'])
        {
            if($_POST['show_in_nav'] == 1)
            {
                ...
                if(empty($nid))
                {
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $ecs->table('nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                    $uri = build_uri('article_cat', array('acid'=> $_POST['id']), $cat_name);
                    //不存在
                    $sql = "INSERT INTO " . $ecs->table('nav') .
                        " (name,ctype,cid,ifshow,vieworder,opennew,url,type) ".
                        "VALUES('" . $cat_name . "', 'a', '" . $_POST['id'] . "','1','$vieworder','0', '" . $uri . "','middle')";
                }
                else
                {
        ...
        $note = sprintf($_LANG['catedit_succed'], $cat_name);
        admin_log($cat_name, 'edit', 'articlecat');
        clear_cache_files();
        sys_msg($note, 0, $link);
