...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    {
        $item_name = filter_compile($_REQUEST['item_name']);
        $item_url = filter_compile($_REQUEST['item_url']);
        $item_ifshow = filter_compile($_REQUEST['item_ifshow']);
        $show_in_nav_pc = filter_compile($_REQUEST['show_in_nav_pc']);
        $item_opennew = filter_compile($_REQUEST['item_opennew']);
        $item_type = filter_compile($_REQUEST['item_type']);
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,show_in_nav_pc) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$show_in_nav_pc')";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,ifshow,vieworder,opennew,url,type,show_in_nav_pc) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$show_in_nav_pc')";
        }
        $db->query($sql);
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $rt['item_show_in_nav_'.$row['show_in_nav_pc']] = 'selected';
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $row = $db->getRow("SELECT ctype,cid,ifshow,type,show_in_nav_pc FROM " . $GLOBALS['ecs']->table('nav') . " WHERE id = '$id'");
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav') .
                " SET name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type',show_in_nav_pc='$show_in_nav_pc' WHERE id='$id'";
        }
        else
        {
            //目标不是分类
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav') .
                " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type',show_in_nav_pc='$show_in_nav_pc' WHERE id='$id'";
        }
