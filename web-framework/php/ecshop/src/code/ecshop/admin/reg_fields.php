...
{
    admin_priv('reg_fields');
    $reg_field_name = filter_compile($_POST['reg_field_name']);
    $reg_field_order = filter_compile($_POST['reg_field_order']);
    /* 检查是否存在重名的会员注册项 */
    if (!$exc->is_only('reg_field_name', filter_compile($_POST['reg_field_name'])))
    {
        sys_msg(sprintf($_LANG['field_name_exist'], filter_compile($_POST['reg_field_name'])), 1);
    }

    $sql = "INSERT INTO " .$ecs->table('reg_fields') ."( ".
                "reg_field_name, dis_order, display, is_need".
            ") VALUES (".
                "'$reg_field_name', '$reg_field_order', '$_POST[reg_field_display]', '$_POST[reg_field_need]')";
    $db->query($sql);

    /* 管理员日志 */
    admin_log(filter_compile($_POST['reg_field_name']), 'add', 'reg_fields');
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $reg_field_order = filter_compile($_POST['reg_field_order']);
    /* 检查是否存在重名的会员注册项 */
    if ($reg_field_name != $_POST['old_field_name'] && !$exc->is_only('reg_field_name', filter_compile($_POST['reg_field_name'])))
    {
        sys_msg(sprintf($_LANG['field_name_exist'], filter_compile($_POST['reg_field_name'])), 1);
    }

    $sql = "UPDATE " .$ecs->table('reg_fields') . " SET `reg_field_name` = '$reg_field_name', `dis_order` = '$reg_field_order', `display` = '$_POST[reg_field_display]', `is_need` = '$_POST[reg_field_need]' WHERE `id` = '$_POST[id]'";
    $db->query($sql);

    /* 管理员日志 */
    admin_log(filter_compile($_POST['reg_field_name']), 'edit', 'reg_fields');
    clear_cache_files();
