...
elseif ($_REQUEST['act'] == 'insert')
{
    /*检查品牌名是否重复*/
    admin_priv('brand_manage');
    $brand_name = filter_compile($_POST['brand_name']);
    $sort_order = filter_compile($_POST['sort_order']);
    $is_show = isset($_REQUEST['is_show']) ? intval($_REQUEST['is_show']) : 0;

    $is_only = $exc->is_only('brand_name', $brand_name);
    ...
    $brand_desc = filter_compile($_POST['brand_desc']);
     /*处理图片*/
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));

    if($_FILES['brand_logo']['name'] == ''){ // 使用品牌外链
        $img_name = $_POST['brand_logo_url'];
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order) ".
           "VALUES ('$brand_name', '$site_url', '$brand_desc', '$img_name', '$is_show', '$sort_order')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order,is_index ".
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    $brand_name = filter_compile($_POST['brand_name']);
    $sort_order = filter_compile($_POST['sort_order']);
    if (filter_compile($_POST['brand_name']) != filter_compile($_POST['old_brandname']))
    {
        /*检查品牌名是否相同*/
        $is_only = $exc->is_only('brand_name', filter_compile($_POST['brand_name']), $_POST['id']);
    ...
    $param = "brand_name = '$brand_name',  site_url='$site_url', brand_desc='$brand_desc', is_show='$is_show', sort_order='$sort_order', is_index='$_POST[is_index]' ";
    if (!empty($img_name))
    {
        //有图片上传
