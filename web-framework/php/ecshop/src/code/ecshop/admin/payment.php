...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    // 如果是微信支付
    if($_REQUEST['code'] == 'wxpay'){

        $store = unserialize($pay['pay_config']);  // 反序列化 将数据取出

        $pay['pay_config'] = $store;
...
elseif (isset($_POST['Submit']))
{
    ...
    // 如果是微信支付
    if($_POST['pay_code'] == 'wxpay'){
        $pay_name = $_POST['pay_name'];
        $pay_desc = $_POST['pay_desc'];
        $pay_fee = $_POST['pay_fee'];
        $data = serialize(['appid'=>$_POST['appid'],'mchid'=>$_POST['mchid'],'key'=>$_POST['key']]);
        $sql = "update ecs_payment set pay_name = '".$pay_name."',pay_desc = '".$pay_desc."',pay_config = '".$data."',pay_fee='".$pay_fee."' where pay_id = '".$_POST['pay_id']."' ";
        $db->query($sql);
