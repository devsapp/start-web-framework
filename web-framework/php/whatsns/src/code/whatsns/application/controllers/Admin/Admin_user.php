...
class Admin_user extends ADMIN_Controller {
	...
	function search() {
		...
		$httpurl=str_replace('admin_user%2Fsearch_html%3F', '', http_build_query($search));
		$httpurl=str_replace('admin_user%2Fsearch.html%3F', '', $httpurl);
		$url=url("admin_user/search")."?".$httpurl;
	
		$departstr = cpage ( $usernum, $pagesize, $page, $url );
	...
	function add() {
		if (null !== $this->input->post ( 'submit' )) {
			...
			$password = strtolower(trim($this->input->post ( 'addpassword' )));
			$email = strtolower(trim($this->input->post ( 'addemail' )));
			$m = $this->user_model->get_by_username ( $this->input->post ( 'addname' ) );
			$u1=$this->db->get_where('user',array('email'=>$email))->row_array();
			if(empty($password)){
				$this->message("密码不能为空");
				exit();
			}
	...
	function edit() {
		...
		if (null !== $this->input->post ( 'submit' )) {
			...
			$conpanyname = $this->input->post ( 'conpanyname' );
			...
			if ($username && '' == $username) {
				...
				$this->db->query ( "UPDATE " . $this->db->dbprefix . "user SET conpanyname='$conpanyname',truename='$truename' WHERE uid=$uid " );
				if ($user ['username'] != $username) {
					...
					$this->db->query ( "UPDATE " . $this->db->dbprefix . "doing SET author='$username' WHERE authorid=$uid " );
					$this->db->query ( "UPDATE " . $this->db->dbprefix . "article_comment SET author='$username' WHERE authorid=$uid " );
					$this->db->query ( "UPDATE " . $this->db->dbprefix . "articlecomment SET author='$username' WHERE authorid=$uid " );
				...
				if ($this->setting ["ucenter_open"]) {
					...
					if(trim($_pwd)!=''&&!empty($_pwd)&&!empty($username)&&$username!=''){
						$this->ucenter_model->uppass ( $this->user ['username'], '',trim($_pwd), $email,1 );						
					}
