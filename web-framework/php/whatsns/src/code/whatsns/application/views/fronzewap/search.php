...
<li class="tagPopup authorinfo">
    ...
    <div class="pages">  {$departstr}</div>
       ...
       {eval  $topiclist=$this->getlistbysql("select id,title,viewtime,articles from ".$this->db->dbprefix."topic where title like '%{$word}%'  order by viewtime desc   limit 0,10");}
        {if $topiclist}
...
<span style="vertical-align:middle;"><a href="{url user/space/$topic['authorid']}">{$topic['author']}   {if $topic['author_has_vertify']!=false}
        <i class="fa fa-vimeo {if $topic['author_has_vertify'][0]=='0'}v_person {else}v_company {/if}  "  ></i>{/if}</a>
                    ...
                    发布于
                                           {eval echo tdate($topic['viewtime']);}</span></li><li class="bookmark " title="{$topic['articles']} 收藏" ></li></ul></div></section>
...
<div class="text-center"  style="margin-bottom:10px;">
      ...
      <ul class="pagination"><li class="next"><a rel="next" href="{url topic/search}?word={$word}">查看更多</a></li></ul>
       </div>
</div>
{/if}
                    </div>
