...
<!--{/if}-->
    ...
    <table class="table" >
        ...
        <tr align="center" class="smalltxt">
            ...
            <button class="btn btn-primary btnshenhe" >审核</button>
            </td>
        </tr>
        <!--{/loop}-->
        <!--{if $departstr}-->
...
<p>提交时间:<span id="m_time"></span></p>
 <button id="comfirm_save" class="btn btn-success mar-t-1">通过</button>
  <button id="comfirm_bohui" class="btn btn-success mar-t-1">驳回</button>
...
$("#comfirm_save").click(function(){
	...
	var _posturl="{url admin_vertifyuser/vertifysave}";
	ajaxpost(_posturl,data,success);
})
//驳回
$("#comfirm_bohui").click(function(){
	$(this).html("点击驳回");

	$("#m_bohui").show();
	var _yuanyin=$.trim($("#m_bohui").val());
	if(_yuanyin==''){
		alert("输入驳回原因!");
		return false;
	}
	...
	var _posturl="{url admin_vertifyuser/bohui}";
	ajaxpost(_posturl,data,success);
})
 </script>
<!--{template footer}-->
