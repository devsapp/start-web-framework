...
class usermodel {
	...
	function get_user_by_username($username) {
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE username='$username'  or email='$username'");
		return $arr;
	}
	...
	function update_uid_byusername($uid, $username) {
		if ($uid == 0) {
			return - 1;
		}
		if ($username == '') {
		...
		$this->db->query ( "UPDATE " . UC_DBTABLEPRE . "members SET uid=$uid WHERE username='$username'" );
		return $this->db->affected_rows ();
	}
