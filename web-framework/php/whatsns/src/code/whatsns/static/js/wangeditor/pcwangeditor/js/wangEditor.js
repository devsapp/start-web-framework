...
_e(function (E, $) {
    ...
    E.fn.updateValue = function () {
        ...
        var wang_txt = editor.txt.wang_txt;

        if ($valueContainer === wang_txt) {
            // 传入生成编辑器的div，即是编辑区域
            return;
        }

        var value = wang_txt.html();
...
_e(function (E, $) {
    ...
    E.fn.initSelection = ieRange ? emptyFn : function () {
        ...
        var range;
        var wang_txt = editor.txt.wang_txt;
        var $firstChild = wang_txt.children().first();
    ...
    E.fn.setRangeByElem = ieRange ? emptyFn : function (elem) {
        var editor = this;
        var txtElem = editor.txt.wang_txt.get(0);
...
_e(function (E, $) {
    ...
    E.fn.getSelfOrParentByName = function (elem, selector, check) {
        ...
        var txt = this.txt.wang_txt.get(0);
...
_e(function (E, $) {
    ...
    function _handle(editor, data, type) {
        ...
        var val = data.val;
        var html = editor.txt.wang_txt.html();
        ...
        // ?????
        // 注释：wang_txt 被重新赋值之后，range会被重置，cloneRange() 也不好使
        // // 重置选区
        // if (range2) {
...
_e(function (E, $) {
    ...
    E.fn.create = function () {
        ...
        // wang_txt 快捷方式
        editor.wang_txt = editor.txt.wang_txt;
    ...
    E.fn.disable = function () {
        this.txt.wang_txt.removeAttr('contenteditable');
    ...
    E.fn.enable = function () {
        ...
        this.txt.wang_txt.attr('contenteditable', 'true');
    ...
    E.fn.clear = function () {
        var editor = this;
        var wang_txt = editor.txt.wang_txt;
        wang_txt.html('<p><br></p>');
        editor.restoreSelectionByElem(wang_txt.find('p').get(0));
    };
...
_e(function (E, $) {
    ...
    MenuContainer.fn.changeShadow = function () {
        ...
        wang_txt.on('scroll', function () {
            if (wang_txt.scrollTop() > 10) {
...
_e(function (E, $) {
    ...
    DropList.fn._position = function () {
        ...
        // 取得 wang_txt 的尺寸
        var txtWidth = editor.txt.wang_txt.outerWidth();
...
_e(function (E, $) {
    ...
    Txt.fn.init = function () {
        ...
        if ($valueContainer.get(0).nodeName === 'DIV') {
            ...
            wang_txt = $valueContainer;
            wang_txt.addClass("wangEditor-txt");
            wang_txt.attr('contentEditable', 'true');
        } else {
    ...
    Txt.fn.contentEmptyHandle = function () {
        var self = this;
        var editor = self.editor;
        var wang_txt = self.wang_txt;
        var $p;

        wang_txt.on('keydown', function (e) {
            if (e.keyCode !== 8) {
                return;
            }
            var txtHtml = $.trim(wang_txt.html().toLowerCase());
            if (txtHtml === '<p><br></p>') {
        ...
        wang_txt.on('keyup', function (e) {
            ...
            if (!txtHtml || txtHtml === '<br>') {
                // 内容空了
                $p = $('<p><br/></p>');
                wang_txt.html(''); // 一定要先清空，否则在 ff 下有问题
                wang_txt.append($p);
    ...
    Txt.fn.bindEnterForDiv = function () {
        ...
        wang_txt.on('keydown keyup', function (e) {
            if (e.keyCode !== 13) {
                return;
            }
            // 查找合法标签
    ...
    Txt.fn.bindFormatText = function () {
        ...
        // 生成 formatText 方法
        wang_txt.formatText = function () {
            var $temp = $('<div>');
            var html = wang_txt.html();
    ...
    Txt.fn.bindHtml = function () {
        ...
        wang_txt.html = function (html) {
            var result;

            if (valueNodeName === 'div') {
                // div 生成的编辑器，取值、赋值，都直接触发jquery的html方法
                result = $.fn.html.call(wang_txt, html);
            }
            ...
            if (html === undefined) {
                ...
                // 手动触发 change 事件，因为 wang_txt 监控了 change 事件来判断是否需要执行 editor.onchange 
                wang_txt.change();
            }
        };
    };
...
_e(function (E, $) {
    ...
    Txt.fn.render = function () {
        var wang_txt = this.wang_txt;
        var $editorContainer = this.editor.$editorContainer;
        $editorContainer.append(wang_txt);
    };
    ...
    Txt.fn.initMaxHeight = function (txtHeight, menuHeight) {
        ...
        if (window.getComputedStyle && 'max-height'in window.getComputedStyle(wang_txt.get(0))) {
            ...
            $wrap.on('scroll', function () {
                if (wang_txt.parent().scrollTop() > 10) {
                    $menuContainer.addClass('wangEditor-menu-shadow');
                } else {
                    $menuContainer.removeClass('wangEditor-menu-shadow');
                }
    ...
    Txt.fn.saveSelectionEvent = function () {
        ...
        wang_txt.on(txtChangeEventNames + ' focus blur', function (e) {
        ...
        // 鼠标拖拽选择时，可能会拖拽到编辑器区域外面再松手，此时 wang_txt 就监听不到 click事件了
        wang_txt.on('mousedown', function () {
            wang_txt.on('mouseleave.saveSelection', function (e) {
                // 先同步后异步，如上述注释
                saveSync();
                saveAync();
            ...
            wang_txt.off('mouseleave.saveSelection');
        });
    ...
    Txt.fn.updateValueEvent = function () {
        ...
        // txt change 时随时更新内容
        wang_txt.on(txtChangeEventNames, function (e) {
            // 初始化
            if (oldValue == null) {
                oldValue = wang_txt.html();
            }
    ...
    Txt.fn.insertEmptyP = function () {
        var wang_txt = this.wang_txt;
        var $children = wang_txt.children();

        if ($children.length === 0) {
            wang_txt.append($('<p><br></p>'));
            return;
        }
    ...
    Txt.fn.wrapImgAndText = function () {
        var wang_txt = this.wang_txt;
        var $imgs = wang_txt.children('img');
        var txt = wang_txt[0];
    ...
    Txt.fn.scrollTop = function (val) {
        ...
        if (editor.useMaxHeight) {
            return wang_txt.parent().scrollTop(val);
        } else {
            return wang_txt.scrollTop(val);
        }
    };
    ...
    Txt.fn.showHeightOnHover = function () {
        ...
        function addAndShowTip($target) {
            ...
            var txtTop = wang_txt.position().top;
            var txtHeight = wang_txt.outerHeight();
        ...
        wang_txt.on('mouseenter', 'ul,ol,blockquote,p,h1,h2,h3,h4,h5,table,pre', function (e) {
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        menu.clickEvent = function (e) {
            ...
            var editor = self.editor;
            var wang_txt = editor.txt.wang_txt;
            var txtOuterHeight = wang_txt.outerHeight();
            var txtHeight = wang_txt.height();
    ...
    E.createMenu(function (check) {
        ...
        $btnSubmit.click(function (e) {
            ...
            if (!isRangeEmpty) {
                ...
                // 去的没有标记的链接，即刚刚插入的链接
                $newLinks = wang_txt.find('a').not('[' + uniqId + ']');
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        function contentHandle($content) {
            ...
            $btnSubmit.click(function (e) {
                ...
                var doHightlight = function () {
                    wang_txt.find('pre code').each(function (i, block) {
                        var $block = $(block);
        ...
        wang_txt.on('keydown click', function (e) {
    ...
    E.createMenu(function (check) {
        ...
        menu.clickEvent = function (e) {
            ...
            if (editor.useMaxHeight) {
                ...
                maxHeight = wang_txt.css('max-height');
                wang_txt.css('max-height', 'none');

                // 如果使用了maxHeight， 将wang_txt从它的父元素中移出来
                $wrapper = wang_txt.parent();
                $wrapper.after(wang_txt);
                $wrapper.remove();
                wang_txt.css('overflow-y', 'auto');
            }
        ...
        menu.clickEventSelected = function (e) {
            ...
            // 还原height
            if (editor.useMaxHeight) {
                wang_txt.css('max-height', maxHeight);
            } else {
    ...
    E.plugin(function () {
        ...
        wang_txt.on('paste', function (e) {
            ...
            if (items) {
                ...
                // 获取
                $imgsBeforePaste = wang_txt.find('img');
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        // 监控 wang_txt drop 事件
        wang_txt.on('drop', function (dragEvent) {
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        var wang_txt = txt.wang_txt;
        var html = '';
        // 说明：设置了 max-height 之后，wang_txt.parent() 负责滚动处理
        var $currentTxt = editor.useMaxHeight ? wang_txt.parent() : wang_txt;
        var $currentTable;
...
_e(function (E, $) {
    E.plugin(function () {
        ...
        function setPosition() {
            ...
            var menuHeight = editor.menuContainer.height();
            var txtHeight = editor.txt.wang_txt.outerHeight();
        ...
        // wang_txt 绑定事件
        wang_txt.on('mouseenter', 'a', function (e) {
            // 延时 500ms 显示toolbar
            if (showTimeoutId) {
        ...
        $toolbar.on('mouseenter', function (e) {
            // 先中断掉 wang_txt.mouseleave 导致的隐藏
            if (hideTimeoutId) {
                clearTimeout(hideTimeoutId);
            }
        }).on('mouseleave', function (e) {
