...
(function (window, $){
	...
	var E = function (textareaId) {
		...
		// textarea
		var wang_teztarea = $('#' + textareaId);
		self.wang_teztarea = wang_teztarea;
...
window.___E_mod(function (E, $) {
	...
	E.fn.addTxt = function () {
		var self = this;
		var wang_teztarea = self.wang_teztarea;
		var val = $.trim(wang_teztarea.val());
			...
			bindEvent: function (editor) {
				...
				menuData.$trigger.on('singleTap', function (e) {
					...
					if (menuData.selected) {
						...
						self.wang_txt.find('blockquote').each(function(key, node){
							// 遍历编辑区域所有的quote
							var $quote = $(node),
		...
		menus[menuId] = {
			...
			showModal: function () {
				...
				var hideModalFn = function (e) {
					...
					// 隐藏modal之后，接着取消事件绑定
					wang_txt.off('singleTap', hideModalFn);
				};

				// 绑定事件
				wang_txt.on('singleTap', hideModalFn);
			},
...
window.___E_mod(function (E, $) {
	...
	E.fn.bindTxtEvent = function () {
		...
		// tap时，记录选区，并显示菜单
		wang_txt.on('focus', function () {
			// 记录编辑器区域已经focus
			self.isFocus = true;
		});
		wang_txt.on('singleTap', function (e) {
			if (self.checkTapTime(e, 'wang_txt') === false) {
				return;
			}
			...
			if ($target.get(0) === wang_txt.get(0)) {
		...
		wang_txt.on('longTap, doubleTap', function () {
			// 隐藏菜单
			self.hideMenuContainer();
		});
		...
		wang_txt.on('blur', function (e) {
			...
			if (menuContainer.contains(explicitOriginalTarget)) {
				// firefox 中，点击菜单会导致 wang_txt blur
...
window.___E_mod(function (E, $) {
	...
	E.fn.bindMenuContainerEvent = function () {
		...
		//tap时，阻止冒泡，因为上层的 wang_txt 会监测 tap 事件
...
window.___E_mod(function (E, $) {
	...
	// 保存、获取 wang_txt tap时event对象的target元素
	...
	E.fn.saveSourceCode = function () {
		...
		var wang_txtClone = wang_txt.clone();
		var $focusElem1 = wang_txtClone.find('.focus-elem');

		// 将当前的 $focusElem 清除样式
		$focusElem1.removeClass('focus-elem');

		// 获取源码
		sourceCode = wang_txtClone.html();
		sourceCode = sourceCode.replace(/\s?class=""/g, '');
		wang_teztarea.val(sourceCode);
	};
...
window.___E_mod(function (E, $) {
	...
	E.fn.setMenuContainerPosition = function () {
		...
		// 编辑区域
		var wang_txt = self.wang_txt;
		var txtTop = wang_txt.offset().top;
		var txtLeft = wang_txt.offset().left;
	...
	E.fn.hideMenuContainer = function () {
		...
		var $focusElem = self.$focusElem;
		var $otherFocusElem = wang_txt.find('.focus-elem'); // 得重新查找，可能发生变化
...
window.___E_mod(function (E, $) {
	...
	E.fn.getWrapRangeElem = function () {
		var self = this;
		var wang_txt = self.wang_txt;
		var txtClass = wang_txt.attr('class');     // 获取编辑区域的class
		...
		// 判断 resultElem 是不是 wang_txt （通过 class 判断）
		if (resultElem.className === txtClass) {
			// 如果 resultElem 正好是 wang_txt
			// 则将 resultElem 试图设置为 wang_txt 最后一个子元素
			resultElem = wang_txt.children().last().get(0) || resultElem;
		}
